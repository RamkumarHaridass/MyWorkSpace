...
def pysparkCompare (baseDf,actualDf,matchedColumns,primaryKey):
    ...
    final_df=out.select([ when(~mismatched_df1[c+'_b'].eqNullSafe(mismatched_df2[c+'_r']),array(mismatched_df1[c+'_b'].cast("string"),\

                mismatched_df2[c+'_r'].cast("string"),mismatched_df1['DfHash_b'].cast("string"))).alias(c) for c in matchedColumns] ) \

                .selectExpr('stack({},{}) as ( colName, mismatch)'.format(len(matchedColumns),','.join('"{0}",`{0}`'.format(c) for c in matchedColumns)))\

                .filter('mismatch is not NULL')

    result=final_df.select(final_df.mismatch[2].alias("key"),"colName",final_df.mismatch[0].alias("baseline_data"),final_df.mismatch[1].alias("actual_data"))
...
def writeMismatchToFile (primaryKey,finalMissMatch,hdfsOutLocation,colDiffReq,fullDiffRequired):
    ...
    if finalMissMatch.head(1):
        ...
        if fullDiffRequired == "true":
            ...
            finalMissMatch.coalesce(1).write.mode("overwrite").option("header","true").format("com.databricks.spark.csv").save(hdfsOutLocation+'/Diffs')
        ...
        colDiffCountCheck.coalesce(1).write.mode("overwrite").option("header","true").format("com.databricks.spark.csv").save(hdfsOutLocation+'/ColumnDiffSum')
...
--keytab /home/pvyc0qap/dvyc0srvhdfsqa.keytab \

--principal dvyc0srvhdfsqa@DEVFG.RBC.COM \
...
class RegressionJob:
    ...
    def compare(self):
            ...
            try:
                ...
                baseline_sql = f'SELECT * FROM {self.baseline_table_name} '                  f'WHERE pcm_batch_id in ({str(baseline_batch_ids)[1:-1]} )'

                print(baseline_sql)

                regression_sql= f'SELECT * FROM {self.regression_table_name} '                   f'WHERE pcm_batch_id in ({str(regression_batch_ids)[1:-1]} )'
                ...
                s3_client=boto3.client(service_name='s3',aws_access_key_id=self.s3_access_key,aws_secret_access_key=self.s3_secret_key,endpoint_url=self.s3_endpoint_url,)
    ...
    def compare_pandas_df(self,baseline_df,regression_df,s3_client,base_record_count,reg_record_count):
               ...
               regression_records= reg_record_count
                ...
                l1=["Test Status","Product Name","Business Date","Baseline PCM Batch ID","Regression PCM Batch ID","Baseline CORE Batch ID","Regression CORE Batch ID","Columns present in Baseline","Columns present in Regression","Records present in Baseline","Records present in Regression","Primary Key Column Count","Baseline Duplicate Records Count","Regression Duplicate Records Count","Mismatched Records Count","Mismatched Columns Count","All Columns Mapped","Unmapped Columns Count"]

                l2=[status,product_name,str(business_date),self.baseline_batch_ids,self.regression_batch_ids,str(baseline_core_batch_id[0]),str(regression_core_batch_id[0]),baseline_columns,regression_columns,baseline_records,regression_records,len(primary_keys),baseline_duplicate_key_count,regression_duplicate_key_count,int(result_df.shape[0]),int(result_df.shape[1]/2),flag,non_matching_cols_count]
    ...
    def compare_pyspark_df(self,baseline_df,regression_df,s3_client,base_record_count,reg_record_count):
                ...
                mismatched_df2[c+'_r'].cast("string"),mismatched_df1['DfHash_b'].cast("string"))).alias(c) for c in cols] ) \

                .selectExpr('stack({},{}) as ( colName, mismatch)'.format(len(cols),','.join('"{0}",`{0}`'.format(c) for c in cols)))\

                .filter('mismatch is not NULL')

                result=final_df.select(final_df.mismatch[2].alias("key"),"colName",final_df.mismatch[0].alias("baseline_data"),final_df.mismatch[1].alias("regression_data"))
                ...
                l1=["Test Status","Product Name","Business Date","Baseline PCM Batch ID","Regression PCM Batch ID","Baseline CORE Batch ID","Regression CORE Batch ID","Columns present in Baseline","Columns present in Regression","Records present in Baseline","Records present in Regression","Primary Key Column Count","Extra Records present in Baseline","Extra Records present in Regression","Baseline Duplicate Records Count","Regression Duplicate Records Count","Mismatched Records Count","Mismatched Columns Count","All Columns Mapped","Unmapped Columns Count"]

                l2=[status,product_name,str(business_date),self.baseline_batch_ids,self.regression_batch_ids,int(base_core_batch_id),int(reg_core_batch_id),int(len(base_list)),int(len(reg_list)),base_record_count,reg_record_count,len(PK),extra_base_count,extra_reg_count,duplicate_base_count,duplicate_reg_count,mismatch_id_count,mismatch_column_count,flag,int(len(non_matching_cols))]